...
export const api = {
  ...
  register: (email: string, password: string, displayName: string) =>
    unwrap<Me>(postApiV1AuthRegister({ body: { email, password, displayName } })),
  ...
  createEvent: (body: { name: string; description?: string; eventDate?: number }) =>
    unwrap<EventItem>(postApiV1Events({ body })),
  ...
  selectPhotos: async (eventId: string, photoIds: string[], selected: boolean) => {
    ...
    for (let index = 0; index < photoIds.length; index += 500) {
      const result = await unwrap<{ updated: number }>(
        postApiV1EventsByEventIdPhotosSelect({
          path: { eventId },
          body: { photoIds: photoIds.slice(index, index + 500), selected },
        }),
      );
  ...
  members: (eventId: string, cursor?: string) =>
    unwrap<Page<EventMember>>(
      getApiV1EventsByEventIdMembers({ path: { eventId }, query: { limit: 50, cursor } }),
    ),
  addMember: (eventId: string, body: { email: string; role: 'admin' | 'member'; displayName?: string }) =>
    unwrap<EventMember & { temporaryPassword?: string }>(
      postApiV1EventsByEventIdMembers({ path: { eventId }, body }),
    ),
  removeMember: (eventId: string, userId: string) =>
    unwrap<void>(deleteApiV1EventsByEventIdMembersByUserId({ path: { eventId, userId } })),
  galleries: (eventId: string, cursor?: string) =>
    unwrap<Page<Gallery>>(
      getApiV1EventsByEventIdGalleries({ path: { eventId }, query: { limit: 50, cursor } }),
    ),
  createGallery: (eventId: string, body: unknown) =>
    unwrap<{ gallery: Gallery; url: string; pin: string }>(
      postApiV1EventsByEventIdGalleries({ path: { eventId }, body } as never),
    ),
  teaser: (slug: string) =>
    unwrap<{
      title: string;
      ...
      unlocked: boolean;
    }>(getApiV1PublicGalleriesBySlug({ path: { slug } })),
  unlock: (slug: string, pin: string) =>
    unwrap<void>(postApiV1PublicGalleriesBySlugUnlock({ path: { slug }, body: { pin } })),
  lock: (slug: string) => unwrap<void>(postApiV1PublicGalleriesBySlugLock({ path: { slug } })),
  publishGallery: (galleryId: string) =>
    unwrap<Gallery>(postApiV1GalleriesByGalleryIdPublish({ path: { galleryId } })),
  unpublishGallery: (galleryId: string) =>
    unwrap<Gallery>(postApiV1GalleriesByGalleryIdUnpublish({ path: { galleryId } })),
  rotatePin: (galleryId: string) =>
    unwrap<{ pin: string }>(postApiV1GalleriesByGalleryIdRotatePin({ path: { galleryId } })),
  galleryPhotos: (slug: string, cursor?: string) =>
    unwrap<Page<Photo>>(
      getApiV1PublicGalleriesBySlugPhotos({ path: { slug }, query: { limit: 48, cursor } }),
    ),
  uploadIntent: (eventId: string, files: { filename: string; contentType: string; fileSize: number }[]) =>
    unwrap<{ uploads: { photoId: string; filename: string; uploadUrl: string }[] }>(
      postApiV1EventsByEventIdPhotosUploadIntent({ path: { eventId }, body: { files } } as never),
    ),
  confirmUpload: (eventId: string, items: { photoId: string; width?: number; height?: number }[]) =>
    unwrap<{ confirmed: number; missing: string[] }>(
      postApiV1EventsByEventIdPhotosConfirm({ path: { eventId }, body: { items } }),
    ),
};
